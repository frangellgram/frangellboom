...
import { EASE_ZONES, FREEZE_HOLD_SECONDS, PULSE_PULLBACK, forwardDuration } from "./boomerangMath";
import { extOf, type BoomerangOptions, type Resolution } from "./boomerang";

// The original ffmpeg.wasm export pipeline, kept alongside the newer
// WebCodecs/mediabunny one (see boomerang.ts) so both can be compared
// side by side from the UI — same BoomerangOptions shape, same output
// contract (a Blob), different engine underneath.
...
// crf 14 — one notch above the 16 this pipeline used to ship with, for the
// quality-vs-quality comparison against the WebCodecs export.
...
export async function createBoomerangFfmpeg(
  ffmpeg: FFmpeg,
  inputFile: File,
